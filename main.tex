...
\begin{equation}
    ...
    \label{eq:SBPFP}
\end{equation}
The goal is to identify $u(x, t)$ that minimizes the `distance' between $p(x)$ and $q(x)$. Now, using Girsanov's theorem (which we do not derive here), we can show that minimizing $\D_{\rm KL}$ is the same as minimizing the quadratic cost associated with the controller.\\

\noindent We can now write then the SBP as,
\begin{align}
    ...
    \text{s.t.}\quad \frac{\partial p(x, t)}{\partial t} + \grad \cdot [u(x, t) p(x, t)] =&\,D \grad^2 p(x, t).
\end{align}
