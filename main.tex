...
\[
\min_{\P_0=p_0, \P_1=p_1} \D_{\rm KL} (\P_t||X_t),
\]
...
\begin{equation}
    \partial_t q(x, t) = D \partial_x^2 q(x, t),
    \label{eq:DiffFP}
\end{equation}
whereas the evolution of $P$ is given by the Fokker-Planck equation,
\begin{equation}
    \partial_t p(x, t) + \partial_x[u(x, t) p(x, t)] = D \partial_x^2 p(x, t).
    \label{eq:SBPFP}
\end{equation}
...
\begin{align}
    ...
    V(t, x) =& \min_u\,\E\bigg[ \frac{1}{4D} \int_t^T |u(x, t)|^2\,\d t \bigg], \\
    u^*(x, t)=&\,\text{arg}\min_u\,V(t, x),\\
    \text{s.t.}\quad\partial_t p(x, t) + \partial_x[u(x, t) p(x, t)] =&\,D \partial_x^2 p(x, t).
\end{align}
...
\]
We can immediately see that the control satisfies,
\[
    u^*(x, t) = -2D \grad V(x, t),
\]
and the evolution equation for the value function becomes,
\[
    \frac{\partial V}{\partial t} + D |\grad V|^2 + D \grad^2 V = 0.
\]
